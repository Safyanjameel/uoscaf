...
import 'Student/order_confirmation.dart';

class OrderFood1Screen extends StatefulWidget {
  @override
  _OrderFood1ScreenState createState() => _OrderFood1ScreenState();
}

class _OrderFood1ScreenState extends State<OrderFood1Screen> {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              SizedBox(height: 8.0),
              Text(
                'Please note that an additional fee of Rs 50 will be charged.',
                style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFFB29222)),
              ),
            ],
          ),
          actions: [
            TextButton(
              child: Text('Cancel', style: TextStyle(color: Color(0xFF01115E),fontWeight: FontWeight.bold),),
              onPressed: () {
                Navigator.pop(context); // Close the dialog
              },
            ),
            TextButton(
              child: Text('Confirm', style: TextStyle(color: Color(0xFF01115E),fontWeight: FontWeight.bold),),
              onPressed: () {
                Navigator.pop(context); // Close the dialog
  ...
  void _performOrderPlacement(String orderDetails, double totalAmount) {
    ...
    FirebaseFirestore.instance
        .collection('orders')
        .add({
      'orderDetails': orderDetails, // Add the extra fee of Rs. 50
    })
        .then((value) {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => OrderConfirmationScreen(
            orderId: value.id,
            totalPrice: totalAmount + 50.0, // Pass the updated total amount
          ),
        ),
      );
    })
